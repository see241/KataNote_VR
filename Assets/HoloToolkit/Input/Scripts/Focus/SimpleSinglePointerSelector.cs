    ...
    public class SimpleSinglePointerSelector : MonoBehaviour, ISourceStateHandler, IInputHandler
    {
        ...
        void IInputHandler.OnInputDown(InputEventData eventData)
        {
            ...
            if (eventData.PressType == InteractionSourcePressInfo.Select)
            {
               ...
               if(instantiatedPointerLine.Handedness == InteractionSourceHandedness.Right)
                {
                    
                    RightGun.GetComponent<RightGunFire>().ShotChick();
                }
               else if(instantiatedPointerLine.Handedness ==  InteractionSourceHandedness.Left)
                {
                    Debug.Log("Left");
                    LeftGun.GetComponent<LeftGunFire>().ShotChlik();
                }
            
            }
            else if(eventData.PressType == InteractionSourcePressInfo.Touchpad) 
            {
